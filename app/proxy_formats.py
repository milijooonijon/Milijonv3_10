"""
Proxy Config Generators and Adapters
Supports: Sing-box (v1.8+ / v1.9+), Clash Meta / Mihomo (YAML), Base64, Plain/Raw, WireGuard
"""
import json
import base64
import urllib.parse
...
def generate_clash_meta_config(nodes: List[Dict[str, Any]]) -> str:
    ...
    for n in nodes:
        ...
        if proto == 'vless':
            ...
            lines = [
                ...
                f'    server: {server}',
                f'    port: {port}',
                f'    cipher: {n.get("cipher", "aes-256-gcm")}',
                f'    password: "{n.get("password")}"',
                '    udp: true'
            ]
            proxy_items.append("\n".join(lines))
    ...
    proxies_section = "\n".join(proxy_items)
    proxies_list_str = ", ".join(proxy_names) if proxy_names else '"DIRECT"'
