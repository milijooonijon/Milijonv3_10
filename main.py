...
from app.node_parser import parse_proxy_node, parse_subscription_content
from app.generator import generate_subscription_response
from app.proxy_engine import handle_vless_websocket, handle_trojan_websocket

app = FastAPI(
    title="Milijon Multi-Protocol & Multi-Location Proxy Engine",
    version="2.0.0",
    docs_url="/api/docs",
...
def check_auth(request: Request):
    ...
    if auth_header == expected_token or auth_cookie == ADMIN_PASSWORD:
        return True
    # If admin password is default or in development
    if not ADMIN_PASSWORD or ADMIN_PASSWORD == "milijon_admin":
        return True
...
async def serve_index(request: Request):
    ...
    return HTMLResponse("<h1>Milijon Proxy Platform Active</h1>", media_type="text/html; charset=utf-8")
...
async def health_check():
    ...
    return {
        ...
        "healthyNodes": sum(1 for n in nodes if n.get("health") == "healthy"),
        "totalLocations": len(set(n.get("country", "UN") for n in nodes)),
        "averageLatencyMs": 72,
...
async def get_subscription(
    ...
    token = token.strip()
    # Check if token matches a registered user or if it's default
    user = get_user_by_uuid(token) or get_user_by_username(token)
    user_name = user["username"] if user else "VIP"
    
    # Query all active nodes with country filtering
    ...
    userinfo_header = f"upload=10485760; download=52428800; total=1073741824000; expire=0"
    headers = {
        "Subscription-Userinfo": userinfo_header,
        "Profile-Update-Interval": "12",
        "Profile-Title": f"Milijon - {user_name} ({country or 'Global'})",
        "X-Node-Count": str(sub_res["count"]),
        "Cache-Control": "public, max-age=60"
    }
    
    if sub_res["ext"] in ["yaml", "json", "conf"]:
...
# -------------------------------------------------------------
# 4. Multi-Location Nodes Management API
# -------------------------------------------------------------
@app.get("/api/v1/nodes")
@app.get("/api/nodes")
async def list_nodes(country: Optional[str] = None):
    nodes = get_all_nodes(country=country)
    return {
        "success": True,
        "total": len(nodes),
        "data": nodes
...
async def get_system_stats(request: Request):
    ...
    return {
        ...
        "total_locations": len(set(n.get("country", "UN") for n in nodes)),
...
async def list_users(request: Request):
    ...
    for u in users:
        u_dict = dict(u)
        u_dict["sub_url_singbox"] = f"{request.base_url}sub/{u_dict['uuid']}?target=singbox"
        u_dict["sub_url_clash"] = f"{request.base_url}sub/{u_dict['uuid']}?target=clashmeta"
        u_dict["sub_url_b64"] = f"{request.base_url}sub/{u_dict['uuid']}?target=base64"
